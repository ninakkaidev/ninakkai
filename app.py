...
def age_verification():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            results = DeepFace.analyze(img_array, actions=['age', 'gender'], enforce_detection=False)
            if not results:
                return jsonify({'success': False, 'error': 'No face detected'}), 400
            prediction = results[0]
            age = prediction['age']
            gender_dict = prediction['gender']
            detected_gender = max(gender_dict, key=gender_dict.get).lower()
            if detected_gender == 'man':
                detected_gender = 'male'
            elif detected_gender == 'woman':
...
def delete_message_endpoint():
    ...
    if not message_id:
        return jupytext({'success': False, 'error': 'No message ID provided'}), 400
    try:
